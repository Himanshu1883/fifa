...
function isRoundOf32MatchNumber(n: number): boolean {
...
async function main() {
  ...
  for (const row of rows) {
    ...
    if (fromLabel !== null && isRoundOf32MatchNumber(fromLabel)) {
      ids.add(row.id);
      continue;
    }
    const fromName = parseMatchNumber(row.name);
    if (fromName !== null && isRoundOf32MatchNumber(fromName)) {
      ids.add(row.id);
    }
  }
  ...
  const result =
    idList.length === 0
      ...
      : await prisma.event.updateMany({
          where: { id: { in: idList } },
          data: { stage: ROUND_OF_32 },
        });

  console.log(
    JSON.stringify({
      rule:
        'Events whose matchLabel OR name (trimmed, entire field) matches /^match\\s*(\\d+)$/i with N in 73..88',
