...
const SEMI_FINALS = "Semi-Finals";
const MATCH_NUMBERS = new Set([101, 102]);
...
function isSemiFinalsMatchNumber(n: number): boolean {
...
async function main() {
  ...
  for (const row of rows) {
    ...
    if (fromLabel !== null && isSemiFinalsMatchNumber(fromLabel)) {
      ids.add(row.id);
      continue;
    }
    const fromName = parseMatchNumber(row.name);
    if (fromName !== null && isSemiFinalsMatchNumber(fromName)) {
      ids.add(row.id);
    }
  }
  ...
  const result =
    idList.length === 0
      ...
      : await prisma.event.updateMany({
          where: { id: { in: idList } },
          data: { stage: SEMI_FINALS },
        });

  console.log(
    JSON.stringify({
      rule:
        'Events whose matchLabel OR name (trimmed, entire field) matches /^match\\s*(\\d+)$/i with N in {101,102}',
