...
const FINAL = "Final";
const MATCH_NUMBERS = new Set([104]);
...
function isFinalMatchNumber(n: number): boolean {
...
async function main() {
  ...
  for (const row of rows) {
    ...
    if (fromLabel !== null && isFinalMatchNumber(fromLabel)) {
      ids.add(row.id);
      continue;
    }
    const fromName = parseMatchNumber(row.name);
    if (fromName !== null && isFinalMatchNumber(fromName)) {
      ids.add(row.id);
    }
  }
  ...
  console.log(
    JSON.stringify({
      rule:
        'Events whose matchLabel OR name (trimmed, entire field) matches /^match\\s*(\\d+)$/i with N === 104',
