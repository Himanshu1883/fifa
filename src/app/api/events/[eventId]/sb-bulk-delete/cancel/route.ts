...
import { cancelBulkDeleteJob, getBulkDeleteJobSnapshot } from "@/lib/sb-bulk-delete-service";
...
export async function POST(req: Request, ctx: { params: Promise<{ eventId: string }> }) {
  ...
  try {
    const result = await cancelBulkDeleteJob(id, jobId);
    if (!result.ok) {
      return NextResponse.json(result, { status: 422 });
    }
    const job = await getBulkDeleteJobSnapshot(jobId);
